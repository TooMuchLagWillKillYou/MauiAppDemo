...
{
    public async Task<List<ReservationDto>> GetAll() => await context.Reservations.Select(r => EntityToDto(r)).ToListAsync();
    public async Task<List<ReservationDto>> GetByDate(DateTime date)
        => await context.Reservations.Where(r => DateOnly.FromDateTime(r.Hour) == DateOnly.FromDateTime(date))
    ...
    public async Task<ReservationDto> Get(int id)
    {
        ...
        if (entity is null)
            throw new ArgumentException($"Couldn't find a reservation with id: {id}");
